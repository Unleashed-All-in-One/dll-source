...
namespace SUC::Player::Evil
{
	...
	void HeadTurnClassic(BBExt::Animation::CAnimationPose* pose, Sonic::Player::CPlayerSpeed* player)
	{
		...
		{
			BBExt::Animation::hkQsTransform* tBone = pose->m_pAnimData->m_TransformArray.GetIndex(EBones::Shoulder_L);
		...
		if(EvilGlobal::allowFreemoveArmRight)
		{
			...
			auto increment = PointToLocalSpace(EvilGlobal::freemovePositionRight, context->m_spMatrixNode->m_Transform.m_Rotation, context->m_spMatrixNode->m_Transform.m_Position) / (11);

			pose->m_pAnimData->m_TransformArray.GetIndex((int)EBones::Arm12Sub_R)->m_Rotation = CQuaternion::Identity();
			...
			for (int i = 0; i < 11; ++i)
			{
				BBExt::Animation::hkQsTransform* tBoneS = pose->m_pAnimData->m_TransformArray.GetIndex((int)EBones::Arm12Sub_R + i);
				tBoneS->m_Rotation = CQuaternion::Identity();
				tBoneS->m_Position = increment;
				tBoneS->m_Scale = CVector(1, 1, 1);;
			}
			pose->m_pAnimData->m_TransformArray.GetIndex((int)EBones::Shoulder_R)->m_Rotation = CQuaternion::Identity();
			pose->m_pAnimData->m_TransformArray.GetIndex((int)EBones::Hand_R_Reference)->m_Position = PointToLocalSpace(EvilGlobal::freemovePositionRight, context->m_spMatrixNode->m_Transform.m_Rotation, context->m_spMatrixNode->m_Transform.m_Position);
			//WorldToLocalScale(context->m_pPlayer->m_spCharacterModel->GetNode("Shoulder_R")->m_WorldMatrix, context->m_pPlayer->m_spCharacterModel->GetNode("Shoulder_R")->m_LocalMatrix, CVector(1, 1, 1));
			for (int i = 0; i < 11; ++i)
			{
				BBExt::Animation::hkQsTransform* tBoneS = pose->m_pAnimData->m_TransformArray.GetIndex((int)EBones::Shoulder_R + i);
				tBoneS->m_Rotation = CQuaternion::Identity();
				tBoneS->m_Scale = CVector(1, 1, 1);;
			}
		}
