...
#include "StateMachine/State/EvilStateArmSwing.h"
#include "AnimationExtension/CeramicSA_AnimatorPose.h"
#include "AnimationExtension/CeramicSA_MiragePose.h"
...
public:
	...
	CVector GetForward()
	{
		return (SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Rotation * Eigen::Vector3f::UnitZ());
	}
	...
	{
		const auto spAnimInfo = boost::make_shared<Sonic::Message::MsgGetAnimationInfo>();
		SONIC_GENERAL_CONTEXT->m_pPlayer->SendMessageImm(SONIC_GENERAL_CONTEXT->m_pPlayer->m_ActorID, spAnimInfo);
		...
		{
			SONIC_GENERAL_CONTEXT->ChangeState("Stand");
			sub_E78310(SONIC_GENERAL_CONTEXT->m_pPlayer);
...
void SpawnParticleOnHand(std::string glitterNameLeft, std::string glitterNameRight, std::string boneless)
{
	...
	{
		if (EvilAttackConfiguration::CLAWPARTICLE == glitterNameLeft)
		{
	...
	{
		if (EvilAttackConfiguration::CLAWPARTICLE == glitterNameRight)
		{
...
void ExecuteAttackCommand(std::string attack, int attackIndex, bool starter = false)
{
	...
	auto resourcelist = EvilAttackConfiguration::animationTable.at(attackIndex).ResourceInfos.Resources;
	int resourceIndex = 0;
	int effectIndex = 0;
...
void SearchThenExecute(std::string input, bool starter, Sonic::EKeyState state)
{
	if (starter)
	{
		for (size_t i = 0; i < EvilAttackConfiguration::starterAttacks.size(); i++)
		{
			if (EvilAttackConfiguration::starterAttacks[i].ActionName == input)
			{
				if (EvilAttackConfiguration::starterAttacks[i].MotionName.empty())
					return;
				comboAttackIndex = i;
				timerCombo = 0;
				playingAttack = true;
				attackCache = EvilAttackConfiguration::starterAttacks[i];
				ExecuteAttackCommand(EvilAttackConfiguration::starterAttacks[i].MotionName, comboAttackIndex, true);
				break;
			}
		}
	}
	else
	{
		...
		{
			if (input == EvilAttackConfiguration::attacks[i].ActionName)
			{
				std::string container;
				if (state == eKeyState_X)
					container = EvilAttackConfiguration::attacks[i].KEY__XDown;
				if (state == eKeyState_Y)
					container = EvilAttackConfiguration::attacks[i].KEY__YDown;
				if (state == eKeyState_A)
					container = EvilAttackConfiguration::attacks[i].KEY__ADown;

				if (container.empty())
					continue;
				ExecuteAttackCommand(container, comboAttackIndex, false);
				for (size_t x = 0; x < EvilAttackConfiguration::attacks.size(); x++)
				{
					///FIX
					if (EvilGlobal::lastAttackName == container)
					{
						attackCache = EvilAttackConfiguration::attacks[x];
						break;
					}
				}
...
{
	if (IS_CLASSIC_SONIC && timerAttack < EvilGlobal::parameters->timerAttackMax && timerCombo < EvilGlobal::parameters->timerComboMax)
	{
		if (playingAttack)
...
HOOK(void, __fastcall, _CAnimationPose_SampleAnimation, 0x6CC950, BBExt::Animation::CAnimationPose* This)
{
...
void HeadTurnClassic(BBExt::Animation::CAnimationPose* pose, Sonic::Player::CPlayerSpeed* player)
{
	using namespace hh::math;
	...
	{	
		BBExt::Animation::hkQsTransform* tBone = pose->m_pAnimData->m_TransformArray.GetIndex(BoneIndex::Shoulder_L);
	...
	//{
		//BBExt::Animation::hkQsTransform* tBone = pose->m_pAnimData->m_TransformArray.GetIndex((int)BoneIndex::Shoulder_R);
		//
		//
		//auto quat = CalculateRotation(SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position, EvilGlobal::freemovePositionRight);
		...
		//for (int i = 0; i < 12; ++i)
		//{			
		//	BBExt::Animation::hkQsTransform* tBoneS = pose->m_pAnimData->m_TransformArray.GetIndex((int)BoneIndex::Shoulder_R + i);
	...
	{
		BBExt::Animation::hkQsTransform* tBoneS = pose->m_pAnimData->m_TransformArray.GetIndex((int)BoneIndex::Shoulder_R - startingPoint + i);
		tBoneS->m_Rotation = CQuaternion::Identity();
		tBoneS->m_Position = increment;
		tBoneS->m_Scale = CVector(1, 1, 1);;
	}
		////auto ref = playerContext->m_pPlayer->m_spCharacterModel->GetNode("Reference");
		//auto increment = (SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position - EvilGlobal::freemovePositionRight) / 11;

		//CVector subtracted;

		//BBExt::Animation::hkQsTransform* tBoneS = pose->m_pAnimData->m_TransformArray.GetIndex((int)BoneIndex::Shoulder_R);
		//tBoneS->m_Position = (increment);
		//subtracted = SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position - (increment);
		//for (size_t i = 1; i < 11; i++) //bones in arm
		//{
		//	BBExt::Animation::hkQsTransform* tBone = pose->m_pAnimData->m_TransformArray.GetIndex((int)BoneIndex::Shoulder_R + i);			
		...
		//BBExt::Animation::hkQsTransform* startBone = pose->m_pAnimData->m_TransformArray.GetIndex(currentBoneIndex);
		//CVector startPosition = startBone->m_Position;
		//
		//// Calculate the target position for the last bone
		//CVector targetPosition = SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position - EvilGlobal::freemovePositionLeft;
		//
		//// Loop through the bones from startBoneIndex to startBoneIndex + numberOfBones - 1
		//for (int i = 0; i < 11; ++i) {
		//	currentBoneIndex = (int)BoneIndex::Shoulder_R + i;
		//	BBExt::Animation::hkQsTransform* tBone = pose->m_pAnimData->m_TransformArray.GetIndex(currentBoneIndex);
		//
		...
		//SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position - ;
		//tBone->m_Position = SONIC_GENERAL_CONTEXT->m_spMatrixNode->m_Transform.m_Position - EvilGlobal::freemovePositionRight;
	//}
	//DebugDrawText::log(std::format("BONE INDEX: {0}", boneIndexTest).c_str(), 0, 100);
	//
	////// RArm
	//BBExt::Animation::hkQsTransform* tRArm = pose->m_pAnimData->m_TransformArray.GetIndex(boneIndexTest);
...
{
	auto test = (BBExt::Animation::CAnimationPose*)player->m_spAnimationPose.get();
...
HOOK(void, __fastcall, _SonicInitializeRenderables_Classic, 0x00DE93A0, app::Player::CPlayerSpeed* This, void*, void* a2, int a3)
{
	...
	//databaseLoader->LoadArchive(database, "EvilSonic.ar", { 100, 0 }, false, true);
...
void RegisterClassicAnimations()
{
	...
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_damageMB", "evilsonic_damageMB");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_guard_idle", "evilsonic_guard_idle");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_BerserkerS", "evilsonic_BerserkerS");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("JumpEvil1", "evilsonic_jumpVS");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("JumpEvil2", "evilsonic_jumpVS2");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_dashS", "evilsonic_dashS");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_dash", "evilsonic_dash");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_start", "evilsonic_start");

	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_runS", "evilsonic_runS");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_idle", "evilsonic_pillar_idle");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_up", "evilsonic_pillar_up", 1, true);
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_upH", "evilsonic_pillar_upH", 1, false);
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_gate", "evilsonic_gate", 1, false);
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_turnL", "evilsonic_pillar_turnL");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_turnR", "evilsonic_pillar_turnR");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_pillar_fall", "evilsonic_pillar_fall", 1, true);
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_run", "evilsonic_run", 3);
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_runE", "evilsonic_runE");
	SUC::System::AnimationSetPatcher::RegisterClassicAnimation("Evilsonic_dash_jumpS", "evilsonic_dash_jumpS");
}
...
HOOK(void*, __fastcall, _ConstructCAnimationPose_Common, 0x006CB140, BBExt::Animation::CAnimationPose* This, void*, void* a2, void* a3)
{
	void* result = original_ConstructCAnimationPose_Common(This, nullptr, a2, a3);
	CAnimationPoseInit_Ctor(This);
	return result;
}
HOOK(void*, __fastcall, _ConstructCAnimationPose_Msn, 0x006CA910, BBExt::Animation::CAnimationPose* This, void*, void* a2, void* a3, void* a4)
{
...
{
	//WRITE_MEMORY(0x015E23F4, char*, "evil_sonic");
	//WRITE_MEMORY(0x015B1A90, char*, "evil_sonic");
	//WRITE_MEMORY(0x015B1AA8, char*, "evil_sonic");
	//WRITE_MEMORY(0x015D0DC8, char*, "evil_sonic");
	//WRITE_MEMORY(0x015D0E88, char*, "evil_sonic");
	//WRITE_MEMORY(0x015D7380, char*, "evil_sonic");
	//WRITE_MEMORY(0x015E2344, char*, "evil_sonic");
	EvilGlobal::canExecuteAttacks = true;
	RegisterClassicAnimations();
	//SUC::System::AnimationSetPatcher::RegisterClassicAnimation("StartEventDash", "evilsonic_runE");
