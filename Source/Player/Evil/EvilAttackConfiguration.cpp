...
std::string EvilAttackConfiguration::CLAWPARTICLE = "slash";
std::string toCheck;
std::string alreadyChecked;
std::vector<Motion> EvilAttackConfiguration::animationTable;
std::vector<std::string> alreadyRegistered;
std::vector<Motion> localMotionFileList;
std::vector<WerehogAttackNew> EvilAttackConfiguration::attacks;
std::vector<WerehogAttackNew> EvilAttackConfiguration::starterAttacks;
////RHand,
//LHand,
//RLeg,
//LLeg,
...
Motion ParseMotionNode(rapidxml::xml_node<>* node)
{
	...
	returned.MotionMoveSpeedRatioFrameStart = std::vector<float>();
	for (rapidxml::xml_node<>* child = node->first_node(); child; child = child->next_sibling())
	{
		const char* name = child->name();
		...
		if (nameS == "Effect")
		{
			for (rapidxml::xml_node<>* child2 = child->first_node(); child2; child2 = child2->next_sibling())
			{
				...
				{
					returned.Effect.REffect_Name1 = EvilAttackConfiguration::CLAWPARTICLE;
				}
				if (isPartOf(name2, "LClaw_Start1") && child2->value() != nullptr)
				{
					returned.Effect.LEffect_Name1 = EvilAttackConfiguration::CLAWPARTICLE;
				}
			}
		}
		if (nameS == "Collision")
...
void RegisterResources(const char* path)
{
	...
	for (size_t i = 0; i < EvilAttackConfiguration::animationTable.size(); i++)
	{
		...
		std::string filepath = std::format("{0}attack/{1}.tbres.xml", modDir, EvilAttackConfiguration::animationTable.at(i).FileName);
		...
		buffer.push_back('\0');
		printf(std::format("\nParsing {0}...", EvilAttackConfiguration::animationTable.at(i).FileName).c_str());
		...
		for (rapidxml::xml_node<>* TriggerBinderResource_Child = resourceNode->first_node(); TriggerBinderResource_Child; TriggerBinderResource_Child = TriggerBinderResource_Child->next_sibling())
		{
			if (isPartOf(TriggerBinderResource_Child->name(), "ResourceInfo"))
			{
				...
				EvilAttackConfiguration::animationTable.at(attackPos).ResourceInfos.Resources.push_back(res);
			}	
			else if (isPartOf(TriggerBinderResource_Child->name(), "TriggerInfo"))
			{
				...
				for (rapidxml::xml_node<>* TriggerInfo_Child = TriggerBinderResource_Child->first_node(); TriggerInfo_Child; TriggerInfo_Child = TriggerInfo_Child->next_sibling())
				{
					...
					EvilAttackConfiguration::animationTable.at(attackPos).TriggerInfos.Resources.push_back(trig);
				}
			}
			id++;			
		}
...
void EvilAttackConfiguration::Install(const char* path)
{
	...
	{
		EvilAttackConfiguration::animationTable.push_back(ParseMotionNode(child));
	}
	...
	for (rapidxml::xml_node<>* child = actionNode->first_node(); child; child = child->next_sibling())
	{
		...
		if (attack.ActionName.find("Start_") != std::string::npos)
			EvilAttackConfiguration::starterAttacks.push_back(attack);
		else
			EvilAttackConfiguration::attacks.push_back(attack);
	}
	auto motionStartpoint = doc2.first_node()->first_node();
	RegisterAnims(EvilAttackConfiguration::animationTable, motionStartpoint);
