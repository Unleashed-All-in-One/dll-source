...
void SubtitleUI::draw()
{
    ...
    if (!m_captionData.m_captions.empty())
    {
        ...
        {
            ImGui::Begin("Textbox", &visible, SubtitleUI::m_hudFlags);
            {
                // Fade in and out
        ...
        ImGui::Begin("Caption", &visible, SubtitleUI::m_hudFlags);
        {
            ImGui::SetWindowFocus();
            ImGui::SetWindowSize(ImVec2(sizeX, sizeY));
        ...
        m_captionData.m_timer += m_captionData.m_isCutscene ? SUC::Project::GetDeltaTime() : SUC::Project::GetHudDeltaTime();
