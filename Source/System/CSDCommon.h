...
public:
	...
	static void PlayAnimation(Chao::CSD::RCPtr<Chao::CSD::CScene>& pScene, const char* name, Chao::CSD::EMotionRepeatType repeatType, float motionSpeed, float startFrame, float endFrame = 0.0f, bool reEnable = false, bool reverse = false)
	{
		PlayAnimation(*pScene.Get(), name, repeatType, motionSpeed, startFrame, endFrame, reEnable, reverse);
	}
	static void FreezeMotion(Chao::CSD::RCPtr<Chao::CSD::CScene>& pScene, float frame = -1)
	{
		FreezeMotion(*pScene.Get(), frame);
	}
	static void StopMotion(Chao::CSD::RCPtr<Chao::CSD::CScene>& pScene, bool end)
	{
		StopMotion(*pScene.Get(), end);
	}
	static void IntroAnim(Chao::CSD::RCPtr<Chao::CSD::CScene>& pScene)
	{
		IntroAnim(*pScene.Get());
	}
	...
	static void CheckSceneAnimation(int i);
	static void Update();
